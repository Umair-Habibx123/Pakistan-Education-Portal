...
export class HeroSectionComponent implements OnInit {
  ...
  getPrograms(educationTypeID: string) {
    this.programService.getPrograms(parseInt(educationTypeID)).subscribe(
      (response) => {
        this.subjects = response;
      },
      (error) => {
  ...
  onSubjectChange() {
    ...
    if (this.tempSelectedSubject) {
      this.getCities(this.tempSelectedSubject);
    }
  }

  getCities(programID: string) {
    this.http.get(`${this.apiURL}school-api/university/getCity?programID=${programID}`).subscribe(
      (response) => {
        this.cities = response;
      },
      (error) => {
